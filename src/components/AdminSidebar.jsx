  ...
  return (
    <aside className="hidden w-64 shrink-0 border-r border-gray-200 bg-gray-50 lg:block dark:border-white/10 dark:bg-navy-800/50">
      <div className="flex h-full flex-col p-4">
        <div className="mb-6 rounded-lg bg-primary-500 px-3 py-2 text-center text-xs font-semibold uppercase tracking-wide text-white">
          Admin Panel
        </div>

        <nav className="flex flex-1 flex-col gap-1">
          {navItems.map((item) => (
            <NavLink key={item.label} to={item.to} end={item.to === '/admin'} className={itemClass}>
              {item.icon}
              {item.label}
            </NavLink>
          ))}
